...
//            console.log(dM.offset().top+dM.innerHeight()-dM.children(':last-child').offset().top-dM.children(':last-child').innerHeight());

        if ( raz ) {
//                console.log ('Пора.');
//                console.log(dM.innerHeight(),dM.children(':last-child').position().top,dM.children(':last-child').innerHeight());
//                console.log(raz);
//                return;
...
//            ddListRequest:false,//URL Запроса для дропдаун.
        ...
        _showErrors: function ( errors ) {
            ...
            $.each( errors, function ( key, val ) {
                ...
                console.log( 'error for (' + '#' + self.options.formName + '-' + key + ')', group );
        ...
        _saveClick: function ( e ) {
            ...
            if ( self.options.requestUrl && self.options.formName ) {
                ...
                $.post( self.options.requestUrl, $.isFunction( self.options.otherParam ) ? self.options.otherParam.call( self, data, self.options.mode ) : $.extend( data, self.options.otherParam ), function ( data ) {
//                    console.log(data);
                    if ( data.status === 'ok' ) {
                                ...
                                $.each( val[3], function ( keyS, valS ) {
                                    ...
                                    var it = $( '<span>' ).attr( {
                                        ...
                                        if ( !parseInt( $( this ).attr( 'data-value' ) ) ) {
                                            console.log( $( this ).parent( ).parent( ).children( 'div' ).length - 1, Object.keys( dt ).length );
                                            ...
                                            console.log( Object.keys( dt ), ( parseInt( $( this ).attr( 'data-value' ) ) - 1 ).toString( ), $.inArray( ( parseInt( $( this ).attr( 'data-value' ) ) - 1 ) + "", Object.keys( dt ) ) );
                                        ...
                                        console.log( $( '#' + self.options.formName + '-' + key ).val( ) );
...
//                            console.log(self.options.hideUnsetedFields);
    ...
    _row: function ( rw, num ) {
        ...
        if ( $.isFunction( this.options.onItemClick ) ) {
            ...
            if ( this.options.onItemClick.length > 1 && $.isFunction( this.options.onItemClick[1] ) ) {
                tr.click( $.extend( {controller: this}, this.options.onItemClick[0] ), this.options.onItemClick[1] );
            }
        }
        return tr;
    },
...
//            console.log(self.options.form);
        ...
        items[items.length] = {
            label: 'Изменить',
            click: function ( ) {
//                            console.log('run edit',self.options.form.fields);
...
//                                    beforeClose:function(){self._refreshOnDown.call(self);}
                } );
            }
        };
        items[items.length] = {
            label: !el.parent().parent().parent().parent().index()
                ...
                :('Удалить запись '),
            click: function ( ) {
                console.log('remove:parent index:',el.parent().parent().parent().parent().index()); 
...
//                    console.log(h,hCnt,Object.keys(fld).length,Object.keys(fld));
...
//            console.log(arr);
    ...
    _checkAndLoad: function ( onEnd ) {
        ...
        var tmp1 = this.table ? this.table.next( ).length ? ( this.table.next( ).position( ).top + this.table.next( ).height( ) )
                ...
                : 0;
        var self = this;
//            console.log(this.table?this.table.children('tbody').children(':last-child').position().top:'Нет таблицы',this.element.height(),tmp1);
        if ( !this.isBusy && tmp1 < this.element.height( ) && this.options.pager.page * this.options.pager.pageSize < this.count ) {
//                console.log('cnl');
            if ( !$.isFunction( onEnd ) ) {
                onEnd = function ( ) {
                    self.isBusy = false
                    //tmp1=this.table?this.table.next().length?(this.table.next().position().top+this.table.next().height()):0:0;
                    tmp1 = this.table ? this.table.next( ).length ? ( this.table.next( ).position( ).top + this.table.next( ).height( ) )
                            : ( this.table.children( 'tbody' ).children( ':last-child' ).length ? this.table.children( 'tbody' ).children( ':last-child' ).position( ).top : 0 )
                            : 0;
//                        console.log(this.table.children('tbody').children(':last-child').position().top,this.element.height(),tmp1);
                    if ( tmp1 < 0 )
                        tmp1 = 0;
...
//                    this.options.pager.page++;????
    ...
    _listIsGet: function ( data, addIt, onEnd ) {
        ...
        this._addTable( );
//            console.log(data,this.options.answerVarName,data[this.options.answerVarName]);
...
//            console.log(this.table.next().position().top,this.element.height());
    },
...
//                    console.log(tb.children(':first-child').children(':first-child'));
            ...
            e.preventDefault( );
//                console.log($(this).parent().prev().children('.content'));
            $( this ).parent( ).prev( ).children( '.content' ).trigger( 'focus' );
        } else if ( e.keyCode === 39 ) {
            e.preventDefault( );
//                console.log($(this).parent().next().prev().children('.content'));
...
//            console.log ('doRequestList: Страница № '+this.options.pager.page+' Размер страницы: '+this.options.pager.pageSize+'шт. Всего: '+this.count);
        if ( this.isBusy )
            return;
        if ( this.hasErr )
            return;
...
//            console.log(this.options);
        if ( this.options.formName || this.options.drawRaw ) {
            ...
            //console.log(reqDt);
            console.log('_doRequestList',this.options);
            let cacheProceed=!(reqDt.materialId || reqDt.parentId);
            let cacheKey=this._doRequestListCreateCacheName(reqDt);
            let cacheDt=cacheProceed?stones.cacheRuntime.getData(cacheKey,'materialTableList'):null;
            if (cacheDt){
                this._listIsGet.call( self, cacheDt, addIt, onEnd );
                stones.cacheRuntime.setData(cacheKey,cacheDt,'materialTableList');
                console.log('list from cache');
            }else{
                self._showLoader( );
                $.post( this.options.requestUrl, reqDt, function ( data ) {
    //                    console.log('doRequetsList',data);
                    self._removeLoader( );
                    self._listIsGet.call( self, data, addIt, onEnd );
                    if (cacheProceed)
                        stones.cacheRuntime.setData(cacheKey,data,'materialTableList');
                } ).fail( function ( answ ) {
                    new m_alert( 'Ошибка сервера', answ.responseText, true, false );
                    console.error( answ );
                } );
...
//                    console.log('tt');
        ...
        _showPicBanner: function ( matId, parentId ) {
            console.log( '_showPicBanner', this, matId );
...
//                console.log(this.content);
                $.each( this.content.children( ), function ( ) {
                    //console.log($(this).find('.active-m').children(':first-child').text());
                    if ( txt )
...
//                console.log(self.postavshik_selected_materials);
                return;
            }
...
//                    console.log(data,self.postavshik_selected_materials);
                    return data;
                },
            } );
            this.element.unbind( 'resize' );
            this.uiDialog.resize( {targetArr: this.m_types}, function ( e ) {
//                console.log(e.data.targetArr.parent.height()-(e.data.targetArr.header.outerHeight()+e.data.targetArr.footer.outerHeight())+'px');
...
//            this.options.minWidth=600;
        ...
        __postChangeStateDo: function ( params ) {
            let {materialId, parentId, postId, coast, optfrom=0, optcoast=0, parentIt, ind}=params;
...
//                console.log(data);
        ...
        _onLastItemClickCreatInput:function(params){
            let {allowPoint=true, curent, dataName, materialId, parentId, value, ind}=params;
            return $( '<input>' ).attr( {
                    type: 'text',
                    'data-name':dataName
                } ).val( value ).change({
...
//                            console.log('activeBack',dt.self.activeBack);
...
//                                console.log('space process');
                ...
                self.selectedVal = {
                    type_id: parentId,
                    mat_id: materialId,
                    firm_id: ( self.postavshik && self.postavshik.attr( 'data-key' ) ) ? self.postavshik.attr( 'data-key' ) : null,
                    coast: self.postavshik_selected_materials[parentId][materialId],
                    optBaseCoast: self.postavshik_selected_materials_opt[parentId][materialId].optcoast,
                    optFrom: self.postavshik_selected_materials_opt[parentId][materialId].optfrom

                };
                console.log( 'selectedVal', self.selectedVal, self.postavshik_selected_materials[parentId] );
            }
        },
        onMaterialItemClick: function ( e ) {
            var self = e.data.self, loader = null, cont = $( this ).parent( ).parent( ).parent( ).parent( ).parent( );
//            self._showPicBanner();
            var ind = e.data.index ? e.data.index : 1;
//            console.log(e.data);
//            console.log($(this));
//            console.log(cont);
            var mainId = $( this ).attr( 'data-key' ), el = $( this );
            var parentId = e.data.parentId ? e.data.parentId : mainId;
            var dt = {
                id: parentId
            };
            ...
            if ( self.options.materialTablesListUrl ) {
                ...
                let cacheData=stones.cacheRuntime.getData(cacheKey,'materialTableList');
                if (cacheData){
...
//                    console.log(data);
                    if ( loader )
                        loader.remove( );
                    var fields = {id: [ 'prim' ]};
                    if ( self.options.subTableDDListUrl ) {
                        fields.name = [ 'string', data.tables[ind - 1].rusname, null, {
                                requestUrl: self.options.subTableDDListUrl,
                                loadPicUrl: self.options.bigLoaderPicUrl,
                                otherParam: {formName: data.tables[ind - 1].fullname}
                            } ];
                    } else {
                        fields.name = [ 'string', data.tables[ind - 1].rusname ];
                    }
//                    if (ind===data.tables.length){
//                        fields.article=['string','Артикл'];
//                    }
                    if ( self.options.referenceColumnName ) {
                        fields[self.options.referenceColumnName] = [ function ( el, defVal ) {
//                            console.log(this,el,defVal);
                    ...
                    var opt = {
                        ...
                        requestUrl: self.options.subtablegetlistUrl,
                        formName: data.tables[ind - 1].fullname,
                        hideColumnName: self.options.referenceColumnName ? self.options.referenceColumnName : [ ],
                        beforeRightClick: function ( e, tr ) {
//                            console.log('beforeRightClick',this);
                            $.each( tr.parent( ).children( ), function ( ) {
                                $( this ).removeClass( 'active-m' );
                            } );
                        },
                        form: {
                            requestUrl: self.options.subtabladdeditUrl,
                            removeUrl: self.options.removeSubTUrl,
                            fields: fields
                        },
                        otherParam: function ( data, inf ) {
                            ...
                            if ( data.treeId.length ) {
                                ...
                                data.treeId = null;
                            //console.log('tmpTreeId',tmpTreeId);
                            if ( self.postavshik_selected_materials ) {
                                console.log( self.postavshik_selected_materials );
                                var ln = $.type( self.postavshik_selected_materials ) === 'object' ?
                                        Object.keys( self.postavshik_selected_materials ).length :
...
//                            console.log(inf,data);
                    ...
                    if ( data.tables.length > ind )
                        opt.onItemClick = [ {self: self, index: ind + 1, parentId: parentId}, self.onMaterialItemClick ];
                    else {
//                        opt.showHeader=true;
                        opt.onItemClick = [ {self: self, index: ind + 1, parentId: parentId, mainId: mainId}, self.onLastItemClick ];
