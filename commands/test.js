    ...
    $.widget( "custom.calculator", $.custom.baseW, {
                ...
                if ( !$.isNumeric( tmpVal ) && $.inArray( tmpVal, self._hardOp ) !== -1 ) {
                    ...
                    switch ( curr ) {
                        ...
                        case'/%':
                            ...
                            break;
                        default:
                            let dop = self._computeAction( rVal, tmpVal, curr );
                            if ( dop !== false )
                                rVal = dop;
                            else
                                return false;
        ...
        _showLine: function () {
            ...
            $.each( this._line, function () {
                ...
                ed = ' ';
                //if (this.val!='sqrt'){
                if ( $.inArray( this.val, self._hardOp ) === -1 && $.inArray( this.cod, self._hardOp ) === -1 )
                    text += this.val;
                switch ( this.cod ) {
                    case'/':
                        text += ' /' + ed;
        ...
        _setInfo: function ( valtmp ) {
            ...
            let s = $.fn.creatTag( 'div', {html: val} );
            this._displayInfo.empty().append( s );
            let d = this._displayInfo.children( 'div:first-child' );
        ...
        _addToLine: function ( cod ) {
            ...
            if ( !this._line.length || $.inArray( codp, this._hardOp ) === -1 || $.inArray( cod, this._hardOp ) === -1 || cod !== codp ) {
                ...
                if ( $.inArray( cod, this._hardOp ) !== -1 )
                    this._line[this._line.length - 1].count = 1;
            ...
            if ( $.inArray( cod, this._hardOp ) !== -1 )
                this._clearAllOnNext = true;
        },
        _popFromLine: function () {
            console.log( this._line );
            if ( this._line.length ) {
                let tmp = this._line.pop();
        ...
        _processKey: function ( dt ) {
            ...
            if ( dt.charCodeAt( 0 ) > 47 && dt.charCodeAt( 0 ) < 58 ) {
                if ( val.length > 11 || ( dt.charCodeAt( 0 ) == 48 && val.charCodeAt( 0 ) == 48 && val.length == 0 ) )
                    return;
                if ( val != '0' )
                    ...
                    case 'del':
                        ...
                        if ( val != '0' ) {
                            if ( val.length === 1 ) {
                                ...
                                if ( val.charCodeAt( val.length - 1 ) == 46 )
                                    this._pointPressed = false;
        ...
        _createButtonBlock: function () {
            let div = $.fn.creatTag( 'div', {'class': 'calculator-keyboard'} );
            let d = $.fn.creatTag( 'tbody', {} );
            let self = this;
            $.each( this._keys, function () {
                let r = $.fn.creatTag( 'tr', {} );
                $.each( this, function () {
                    let a = $.fn.creatTag( 'a', {'class': 'btn', html: this.l, role: 'calckey', data: this.cod} );
                    let td = $.fn.creatTag( 'td', {} );
                    a.click( function ( e ) {
            ...
            div.append( $( '<table>' ).append( d ) );
            return div;
        },
        _memUpdateState: function () {
            let cont = this._display.children( 'span' );
            if ( this._mem !== null ) {
                if ( !cont.length ) {
                    cont = $.fn.creatTag( 'span', {text: 'M'} );
                    let d = this._display.children( 'div:first-child' );
                    if ( d.length )
                        d.before( cont );
                    else
        ...
        _memClear: function () {
            //&:nth-child(1);
            let m = this.dialog.find( '.calculator-memory' );
